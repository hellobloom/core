...
contract('BatchInitializer', function([owner, admin, unrelated]) {
  ...
  describe('Configuration', async () => {
    it('allows the owner to end initialization in attestation logic', async () => {
      await batchInitializer.endInitialization(attestationLogic.address, {
        from: owner,
      }).should.be.fulfilled
    })
    it('allows the owner to end initialization in registry', async () => {
      await batchInitializer.endInitialization(registryLogic.address, {from: owner})
        .should.be.fulfilled
    })

    it('Fails if address points to unrelated contract', async () => {
      await batchInitializer
        .endInitialization(tokenEscrowMarketplace.address, {from: owner})
        .should.be.rejectedWith(EVMThrow)
    })

    it('does not allow anyone else to end initialization', async () => {
      await batchInitializer
        .endInitialization(registryLogic.address, {from: unrelated})
        .should.be.rejectedWith(EVMThrow)
    })
    ...
    it('allows the owner to configure contracts', async () => {
      ...
      await batchInitializer.setTokenEscrowMarketplace(unrelated, {from: owner})
        .should.be.fulfilled
      await batchInitializer.setAttestationLogic(unrelated, {from: owner}).should.be
        .fulfilled
      await batchInitializer.setRegistryLogic(unrelated, {from: owner}).should.be
        .fulfilled
    })
    ...
    it('Does not allow anyone else to set interface contracts', async () => {
      await batchInitializer
        .setAdmin(unrelated, {from: admin})
        .should.be.rejectedWith(EVMThrow)
      await batchInitializer
        .setTokenEscrowMarketplace(unrelated, {from: admin})
        .should.be.rejectedWith(EVMThrow)
      await batchInitializer
        .setAttestationLogic(unrelated, {from: admin})
        .should.be.rejectedWith(EVMThrow)
      await batchInitializer
        .setRegistryLogic(unrelated, {from: admin})
        .should.be.rejectedWith(EVMThrow)
    })
  })
  ...
  describe('Batch loading accounts', async () => {
    it('allows the admin to batch load accounts links', async () => {
      const numlinks = 50
      const currentAddresses = testLinks
        .slice(0, numlinks)
        .map(a => a.currentAddress)
      const newAddresses = testLinks.slice(0, numlinks).map(a => a.newAddress)
      await batchInitializer.batchLinkAddresses(currentAddresses, newAddresses, {
        from: admin,
      }).should.be.fulfilled
      let i = 0
    ...
    it('Fails if initialization ended', async () => {
      ...
      await batchInitializer.endInitialization(registryLogic.address, {from: owner})
        .should.be.fulfilled
      await batchInitializer
        .batchLinkAddresses(currentAddresses, newAddresses, {from: admin})
        .should.be.rejectedWith(EVMThrow)
    })
    ...
    it('Does note allow anyone else to batch load', async () => {
      ...
      const newAddresses = testLinks.slice(0, numlinks).map(a => a.newAddress)
      await batchInitializer
        .batchLinkAddresses(currentAddresses, newAddresses, {from: unrelated})
        .should.be.rejectedWith(EVMThrow)
    })
  ...
  describe('Batch loading attestations', async () => {
    it('allows the admin to batch load attestations', async () => {
      ...
      const requesters = testAttestations
        .slice(0, numAttestations)
        .map(a => a.requester)
      const attesters = testAttestations
        .slice(0, numAttestations)
        .map(a => a.attester)
      const subjects = testAttestations.slice(0, numAttestations).map(a => a.subject)
      const dataHashes = testAttestations
        .slice(0, numAttestations)
        .map(a => a.dataHash)
      await batchInitializer.batchMigrateAttestations(
        requesters,
        attesters,
        subjects,
        dataHashes,
    ...
    it('Fails if initialization ended', async () => {
      ...
      await batchInitializer
        .batchMigrateAttestations(requesters, attesters, subjects, dataHashes, {
          from: admin,
        })
        .should.be.rejectedWith(EVMThrow)
    })
    ...
    it('Fails if lengths of args not equal', async () => {
      ...
      const attesters = testAttestations
        .slice(0, numAttestations)
        .map(a => a.attester)
      const subjects = testAttestations
        .slice(0, numAttestations - 1)
        .map(a => a.subject)
      const dataHashes = testAttestations
        .slice(0, numAttestations)
    ...
    it('Fails if lengths of args not equal', async () => {
      ...
      const requesters = testAttestations
        ...
        .map(a => a.requester)
      const attesters = testAttestations
        .slice(0, numAttestations - 3)
        .map(a => a.attester)
      const subjects = testAttestations
        .slice(0, numAttestations - 2)
        .map(a => a.subject)
      const dataHashes = testAttestations
        .slice(0, numAttestations - 1)
        .map(a => a.dataHash)
